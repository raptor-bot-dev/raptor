-- =============================================================================
-- RAPTOR v3.4 Migration 018: Fix position status check in reserve_trade_budget
--
-- Bug: reserve_trade_budget was checking status='OPEN' but positions are created
--      with status='ACTIVE' (required by DB constraint).
-- Impact: Position count/exposure queries returned 0, bypassing max_positions limit.
-- Fix: Change status='OPEN' to status='ACTIVE' on lines 204 and 220.
-- =============================================================================
...
BEGIN
  ...
  IF FOUND THEN
    IF v_existing_execution.status IN ('CONFIRMED', 'CANCELED') THEN
    ...
    IF v_existing_execution.status = 'FAILED' THEN
  ...
  IF p_mode = 'AUTO' THEN
    ...
    -- FIX: Changed status='OPEN' to status='ACTIVE' (positions use ACTIVE status)
