...
import type { ChainConfig, Opportunity, Position } from '@raptor/shared';
import {
  createPosition,
  recordTrade,
  closePosition,
...
export class ChainExecutor {
  ...
  private async executeBuy(
    ...
    const entryPrice = Number(amount) / Number(tokensReceived);
    ...
    const position = await createPosition({
      ...
      token_address: token,
      token_symbol: symbol,
      amount_in: ethers.formatEther(amount),
  ...
  async executeSell(
    ...
    const exitPrice = Number(amountOut) / Number(tokensHeld);
    ...
    await closePosition(positionId, {
      ...
      pnl: ethers.formatEther(amountOut),
    ...
    await recordTrade({
      ...
      amount_out: ethers.formatEther(amountOut),
